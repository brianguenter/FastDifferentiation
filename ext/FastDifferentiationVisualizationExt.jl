...
function make_dot_file(graph, edges_to_draw::AbstractVector{P}, label::String, reachability_labels=true, value_labels=true, no_path_edges=false) where {P<:PathEdge}
    ...
    for e in edges_to_draw
        ...
        temp = node(gr_copy, top_vertex(e))
        nodes_to_draw[temp] = temp #have to use IdDict rather than Set because == doesn't return a Boolean in latest version of FastDifferentiation
        temp = node(gr_copy, bott_vertex(e))
        nodes_to_draw[temp] = temp
    end
